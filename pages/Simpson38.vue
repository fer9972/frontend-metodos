<template>
  ...
  <div>
    ...
    <v-form ref="formRegister" v-model="formRegister" lazy-validation>
      ...
      <v-container>
        <h1>Metodo Simpson 3/8 Con una función</h1>
        <p class="text-lg-left">
          Por Favor ingrese la función que desea calcular, y los parámetros X0 y
          Xn
        </p>
        <v-row>
          <v-col cols="12" sm="6">
      ...
      <v-btn class="mx-8" @click="simpson38()"> Calcular integral </v-btn>
  ...
  methods: {
    simpson38() {
      ...
      if (this.$refs.formRegister.validate() && this.formRegister) {
        const url = 'https://backend-metodos.herokuapp.com/simpson38'
        let data = {}
        ...
        this.$axios
          ...
          .then((res) => {
            ...
            let funcion = this.funcion.funcion.toString()
